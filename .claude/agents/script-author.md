...
- **Deployment:** a new script that should land in `$HOME`/`PATH` needs an `ext-dotfiles.nix` mapping on the nix hosts (flag the exact line to the user) or `stow` on others — coordinate with `dotfiles-architect`.
