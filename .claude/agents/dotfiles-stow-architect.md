...
You have a persistent, file-based memory system. The path depends on the current platform:
...
Detect which platform you're on by checking the working directory or `$HOME` prefix, then use the corresponding path. This directory already exists on both platforms — write to it directly with the Write tool (do not run mkdir or check for its existence).
